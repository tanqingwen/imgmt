				...
				<div class="row clearMar" >
					...
					<div class="content">
						<div class="main">
							...
							<form  action="${ctx}/staff/searchVenueResultList" class="form-horizontal" method="post">
										...
										<div class="col-lg-6 col-md-6">
											<div class="form-group ">
												...
												<select name="mmPmtMode" id="mmPmtMode" class="venueGrade" >
				 									...
				 									<option value="1"  <c:if test="${mmPmtMode eq 2 }">selected="selected"</c:if>>二级 - 场馆</option>
				 									<option value="2"  <c:if test="${mmPmtMode eq 3 }">selected="selected"</c:if>>三级 - 子场馆</option>
				 								</select>
											</div>
										</div>
									</div>
												...
												<tr>
				                                    ...
				                                    <td class="operationAll">
				                                    	<c:if test="${app:checkPermission('VENUE_VIEW') }">
				                   	                 		<a href="${ctx}/staff/venuePreserve_view/${item.mmMerchantNo}" ><span class="edit">场馆详情</span></a>
				                                    	</c:if>
				                                    	<c:if test="${app:checkPermission('VENUE_MLIST') }">
				                                    		<a href="${ctx}/staff/venuePreserve_list/${item.mmMerchantNo}"><span class="check">商户列表</span></a>
				                                    	</c:if>
				                                    	<c:if test="${app:checkPermission('VENUE_GLIST') }">
				                                    		<a href="${ctx}/staff/gatePreserve_list/${item.mmMerchantNo}"> <span class="delete">闸机列表</span> </a>
				                                    	</c:if>
				                                    </td>
												</tr>
											</c:forEach>
								...
								<%-- <div class="box-footer clearfix">
									<tags:pagination url="${ctx}/staff/searchVenueResultList" 
