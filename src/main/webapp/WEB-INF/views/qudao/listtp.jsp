						...
						<div class="main">
							...
							<form id="defaultForm" name="defaultForm" class="form-horizontal" action="" method="post">
										...
										<div class="col-lg-6 col-md-6">	
											...
											<label for="ctApproveTimeStart" class="labelWidth">结束时间：</label>										
												<div class="input-group date firstCommission" style="display:inline-table;vertical-align:middle;margin-left:8px;">
													...
													class="form-control dateWidth line-input" style="margin-left:0;width:250px;">
													<input type="hidden" id="endTime" name="endTime" value="${cpShiftDate }"> 
												</div>
											</div>
										</div>
									</div>
										...
										<table class="table table-responsive">
											...
											<tbody>
											 <c:forEach var="var" items="${list }" varStatus="status">
												<tr>
												<td>${var.ticketno }</td>
												<td>${var.returnTime }</td>
												<td> 
													<c:if test="${var.refundChannels eq '1' }">微信公众号</c:if> 
													<c:if test="${var.refundChannels eq '2' }">APP</c:if> 
													<c:if test="${var.refundChannels eq '3' }">现场</c:if> 
													<c:if test="${var.refundChannels eq '4' }">官网</c:if> 
													<c:if test="${var.refundChannels eq '5' }">驴妈妈</c:if> 
													<c:if test="${var.refundChannels eq '6' }">携程</c:if> 
													<c:if test="${var.refundChannels eq '7' }">途牛</c:if> 
													<c:if test="${var.refundChannels eq '8' }">同程</c:if> 
													<c:if test="${var.refundChannels eq '9' }">淘宝旅行</c:if> 
													<c:if test="${var.refundChannels eq '10' }">终端</c:if> 
													<c:if test="${var.refundChannels eq '11' }">闸机</c:if> 
													<c:if test="${var.refundChannels eq '12' }">智能pos</c:if> 
												</td> 
													<td>${var.refundAmount }</td>
													<td>${var.sonorderlist }</td>
													<td>${var.orderPayType }</td>
													<td>${var.orderMoney }</td>
													<td>${var.orderName }</td> 
													<td>
								...
								<div class="col-md-12  marginBottom">
									...
									<tags:pagination url="${ctx}/qudao/listtp"   page="${cpShift}" cssClass="pull-right"/>
								</div>
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">   $(function() {
       ...
       $('#ctApproveTimeStart,#ctApproveTimeEnd').datepicker(dataPickerOp2);
		...
		$("#theIdForSubmit").click(function() {
			$("#defaultForm").attr("action","${ctx }/qudao/listtp");
			$("#downLoad").attr("disabled",false);
		});							
		
		$("#downLoad").click(function(){
			$("#defaultForm").attr("action","${ctx}/qudao/isDownLoad2");	
		});
