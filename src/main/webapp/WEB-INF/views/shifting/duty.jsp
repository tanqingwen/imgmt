			...
			<div class="container">
				<div class="row">
					<div class="col-lg-1 col-md-1 text-center lebelT">
						<img src="${assets }/app/img/收银交班.png" />
					</div>
							...
							<div class="contentInner clearfix">
								...
								<div class="col-lg-12">
									<div class="col-lg-6 banbiehao">
										<%-- <dl>
										<dd>班别号: <span>2017-06-01-002</span></dd>
										<dd>打印时间: <span>2017-06-01 20:05</span></dd>
										<dd>结班员工工号: <span>${userId }</span></dd>
									</dl> --%>
										<dl>
											<dd>
												班别号: <span id="year"></span>
											</dd>
											<!-- <dd>
												打印时间: <span id="printTime"></span>
											</dd> -->
											<dd>
												交班员工工号: <span>${userId }</span>
											</dd>
											<dd>
												交班员工: <span>${userName }</span>
											</dd>
										</dl>
									</div>
									<div class="col-lg-6 banbiehao">
										<dl>
											<dd>
												交班时间: <span id="jbTime"></span>
											</dd>
											<dd>
												设备名称: <span>${posIp }</span>
											</dd>
										</dl>
									</div>
								</div>
								...
								<div class="col-lg-12">
									...
									<div class="col-lg-6 banbiehao">
										...
										<tr>
											...
											<%-- <c:forEach items="${list2}" var="money">
											<tr>
											<td style="width: 38%;">
												<c:if test="${money.payType=='cash' }">现金</c:if>
												<c:if test="${money.payType=='xc_wx' }">微信</c:if>
												<c:if test="${money.payType=='xc_zfb' }">支付宝</c:if>
											</td>
											<td style="width: 10%;">:</td>
											<td style="width: 30%;"></td>
											<td style="width: 20%;text-align:right;">${money.payMoney}</td>
										</tr>
										</c:forEach> --%>
											<c:forEach items="${list2 }" var="map">
												<tr>
													...
													<td style="width: 30%;"></td>
													<td style="width: 20%; text-align: right;">
													<c:out value="${map.value }"></c:out></td>
												</tr>
											</c:forEach>
											<tr>
									...
									<%-- <div class="col-lg-6 banbiehao">
										...
										<tr>
											...
											<c:forEach items="${recharge }" var="re">
												<tr>
													<td style="width: 38%;"><c:if
															test="${re.key == 'POS' }">信用卡</c:if> <c:if
															test="${re.key == 'CASH' }">现金</c:if> <c:if
															test="${re.key == 'XC_WX' }">微信</c:if> <c:if
															test="${re.key == 'XC_ZFB' }">支付宝</c:if></td>
													<td style="width: 10%;">:</td>
													<td style="width: 30%;"></td>
													<td style="width: 20%; text-align: right;"><c:out
															value="${re.value }"></c:out></td>
												</tr>
											</c:forEach>
											<tr>
									...
									<div class="col-lg-6 banbiehao">
										...
										<table border="0" cellspacing="0" cellpadding="0"
											...
											<tr>
												...
												<td style="width: 10%;">:</td>
												<td style="width: 30%;"></td>
												<td style="width: 20%; text-align: right;">${cash }</td>
											</tr>
										</table>
								...
								<div class="freebtn text-right col-md-12 clearfix" style="margin-bottom:20px;">
										<button type="button" id="bt"
											onclick="javascript:doPrint('myDiv')" class="btn-green"
											>打印</button>
								</div>
	...
	<script language="javascript" type="text/javascript">
		...
		function doPrint(printDiv) {
			...
			try {
				...
				newwin.document.body.innerHTML = document
				...
				//newwin.window.print();
				newwin.document.getElementById("PrintCommandObject").ExecWB(6,
						1);
				newwin.window.close();
				pagesetup_default();
						...
						<div class="contentInner" style="padding: 10px 20px;">
							...
							<div class="col-lg-12" style="width: 100%; margin: 20px 0;">
								<div class="col-lg-6 banbiehao"
									...
									<dl>
										<dd>
											班别号: <span id="dyear"></span>
										</dd>
										<dd>
											打印时间: <span id="dprintTime"></span>
										</dd>
										<dd>
											交班员工工号: <span>${userId }</span>
										</dd>
								...
								<div class="col-lg-6 banbiehao"
									...
									<dl>
										<dd>
											交班时间: <span id="djbTime"></span>
										</dd>
										<dd>
											POS机号: <span>${posIp }</span>
										</dd>
										<dd>
											交班员工: <span>${userName }</span>
										</dd>
							...
							<div class="clearfix"
								style="contet: ''; clear: both; display: block;"></div>
							<div class="col-lg-12" style="width: 100%; margin: 20px 0;">
								<div class="col-lg-6 banbiehao"
									style="width: 49%; display: inline-block; float: left;">
									<h4>当班营业明细</h4>
								...
								<div class="col-lg-6 banbiehao"
									...
									<table border="0" cellspacing="0" cellpadding="0"
										...
										<c:forEach items="${list2 }" var="map">
											<tr>
												<td style="width: 38%;"><c:if
														test="${map.key =='XC_ZFB' }">支付宝</c:if> <c:if
														test="${map.key =='XC_WX' }">微信</c:if> <c:if
														test="${map.key =='POS' }">信用卡</c:if> <c:if
														test="${map.key =='CASH' }">现金</c:if> <c:if
														test="${map.key =='PUBLIC' }">公关卡</c:if> <c:if
														test="${map.key =='RECHARGE' }">充值卡</c:if></td>
												<td style="width: 10%;">:</td>
												<td style="width: 30%;"></td>
												<td style="width: 20%; text-align: right;"><c:out
							...
							<div class="col-lg-12"
								style="margin-top: 10%; width: 100%; margin: 20px 0;">
								<div class="col-lg-6"
									style="width: 49%; display: inline-block; float: left;">
									<div class="col-lg-6"
										style="margin: 0 auto; width: 49%; float: left;">
										当班收银: <span>${userName }</span>
									</div>
									<!-- <div class="col-lg-6"
										style="margin: 0 auto; width: 49%; float: left;">
