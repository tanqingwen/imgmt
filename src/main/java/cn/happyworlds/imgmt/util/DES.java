...
public class DES {
	...
	public static synchronized String DES_1(String source, String key, int type) {

		if (source.length() % 16 !=0 || key.length() % 16!=0)
			return null;
		StringBuffer sb = new StringBuffer();
		if (type == 0) {
			
			for (int i = 0; i < source.length()/16; i++) {
				sb.append(encryption(source.substring(i*16,i*16+16), key));
			}
			return sb.toString();
		...
		if (type == 1) {
			...
			for (int i = 0; i < source.length()/16; i++) {
				sb.append(discryption(source.substring(i*16,i*16+16), key));
			}
			return sb.toString();
	...
	public static synchronized String DES_3(String source, String key, int type) {

		if (key.length() % 16 !=0 || source.length() % 16 !=0)
			return null;
		...
		for (int i = 0; i < source.length()/16; i++) {
			if (type == 0) {
				...
				temp = encryption(source.substring(i*16,i*16+16), K1);
				...
				temp = discryption(source.substring(i*16,i*16+16), K1);
	
				temp = encryption(temp, K2);
	
				temp = discryption(temp, K1);
	...
	//分散数据
	public static String dataDistributed(String source,String key){
		if (key.length() % 32 !=0 || source.length() % 16 !=0)
			return null;
		String d1 = DES_3(source, key, 0);
		String d2 = DES_3(reverse(source), key, 0);
		return d1+d2;
	} 
	//MAC
	public static String mac(String mak, byte[] data){
		//做异或
		byte[] b1 = xor(data, 8);
		//转16进制
		String h1 = Codec.hexEncode(b1);
		//取前8个字节用MAK加密
		String r1 = DES.DES_1(DES.ASC_2_HEX(h1.substring(0,8)), mak, 0);
		//加密后的结果与后8个字节异或
		byte[] b2 = xor(Codec.hexDecode(r1), h1.substring(8).getBytes());
		String r2 = DES.DES_1(Codec.hexEncode(b2), mak, 0);
	...
	public static String ASC2Binary(String hexString)  
    {  
...
//		System.out.println(divData("1111111111111111", "11111111111111111111111111111111", HEX));
//		String keystr[] =new String[]{
//				"0d2279e5ebad62c90cd42a3f36ed6ba8"
//				,"c6654a2d077a2edd32adf680cb63fc2d"
//				,"430125a5facec404b62bd5fff8a8fded"
//				};
//		String datastr ="3030303030303031";
//		for (int i = 0; i < keystr.length; i++) {
//			String s= dataDistributed(datastr, keystr[i]);
//			System.err.println(i+2+": "+s.substring(0,12)+" "+s.substring(s.length()-12,s.length()));	
//		}
//
//		String s =dataDistributed("3503000188888880", "196f6aa1134bd2cc685f30591fc7589b");
//		System.err.println("1: "+"            "+" "+s.substring(s.length()-12,s.length()));
